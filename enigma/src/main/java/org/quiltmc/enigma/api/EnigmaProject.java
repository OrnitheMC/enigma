...
public class EnigmaProject {
	public static boolean doFastRenamableMethodCheck; // it's jank but I can't be arsed rn
	...
	public boolean isRenamable(Entry<?> obfEntry) {
		...
		if (obfEntry instanceof MethodEntry method) {
			...
			if (this.libIndex.getChildrenByClass().get(new ClassEntry("java/lang/Object")).stream().anyMatch(c -> c instanceof MethodEntry m && m.compareTo(method) == 0)
					|| this.libIndex.getChildrenByClass().get(new ClassEntry("java/lang/Record")).stream().anyMatch(c -> c instanceof MethodEntry m && m.compareTo(method) == 0)) {
				return false;
			}
			...
			if (parent != null && parent.isEnum()
					...
					|| isEnumValueOfMethod(parent, method))) {
				return false;
			}
