package org.quiltmc.enigma.gui.warning;

import org.quiltmc.enigma.api.analysis.EntryReference;
import org.quiltmc.enigma.api.analysis.index.jar.EntryIndex;
import org.quiltmc.enigma.gui.Gui;
import org.quiltmc.enigma.gui.GuiController;
import org.quiltmc.enigma.gui.TooltipEditorPane;
...
public class WarningChecker {
	...
	public boolean checkOverwrittenMethods(EntryReference<Entry<?>, Entry<?>> reference, EntryRemapper mapper) {
		Collection<MethodEntry> methods = this.controller.getProject().getJarIndex().getIndex(EntryIndex.class).getMethods();
		for (MethodEntry methodEntry : methods) {
			if (Objects.equals(methodEntry.getName(), reference.entry.getName())) {
				String methodName = mapper.getMapping(methodEntry).targetName();
				if (methodName != null) {
	...
	public boolean followsStyle(EntryReference<Entry<?>, Entry<?>> reference) {
		...
		String name = mapper.getMapping(reference.entry).targetName();
		if (name != null) {
			...
			String parentName = mapper.getMapping(parent).targetName();
			if (parentName != null) {
	...
	public boolean followsFieldStyle(EntryReference<Entry<?>, Entry<?>> reference, String name) {
		EntryIndex entryIndex = this.controller.getProject().getJarIndex().getIndex(EntryIndex.class);
